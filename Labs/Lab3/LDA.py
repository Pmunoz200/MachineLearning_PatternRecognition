...
def covariance_within_class(matrix_values, label, class_labels):
    ...
    for i in range(len(class_labels)):
        ...
        cov_matrix = ML.covariance(centered_matrix)
...
if __name__ == '__main__':
    ...
    [attributes, labels] = ML.loadCSV(pathname, class_label, attribute_names)
    [Sw, Sb] = between_within_covariance(attributes, labels, class_label)
    [W, _] = ML.LDA1(attributes, labels, class_label, m)
    P = np.dot(W.T, attributes)
    ML.graphic_scatter_2d(P, labels, class_label)
